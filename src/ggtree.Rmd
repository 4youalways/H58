...
# tree %>% as_tibble() %>% filter(label == "AL513382.1")
# 
# MRCA(tree, 257)
# tree
# y <- groupClade(tree, 257)
# 
# ggtree(y, aes(linetype = group)) + scale_x_break(c(0.00001, 0.00013), expand = F) + theme_bw() + hexpand(0.000009, direction = -1)#%<+% data

p <- 
  ggtree(y) %<+% data + scale_x_break(c(0.00001, 0.00013), expand = F) +  hexpand(0.00009, direction = -1) + hexpand(0.09, direction = 1) + theme_dark() #+ xlim(NA, 0.00013)
# p
# hehe
# gheatmap(hehe, year, width = 0.005)
# parent(tree, 127)
# p + geom_nodepoint()
...
p3 #<-
  ...
  #hexpand(.02) +
  vexpand(vertical_expansion) + xlim(-.0000005, 0.000145) + theme_dark()
