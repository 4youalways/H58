...
p <- 
  ...
  #drop.tip("AL513382.1") %>%
  ggtree(cex = .2) %<+% data +
  geom_tippoint(aes(color=genotype), size = 0.1) +
...
p3 <-
    gheatmap(p, year,
        ...
        font.size = font_size,
        legend_title = "Year") +
      scale_fill_grey(end = 1, na.translate = FALSE) +
      #scale_fill_manual(c()) +
...
p5 <-
    gheatmap(p4, group,
        ...
        font.size = font_size,
        legend_title = "Group") +
  #scale_fill_viridis_d(aesthetics = "fill", option = "H") +
  scale_fill_manual(values = value_colors, na.translate = F)+
    labs(fill = "Year") +
  hexpand(.2) +
  vexpand(vertical_expansion) +
...
p7 <-
    gheatmap(p6, region,
        ...
        font.size = font_size,
        legend_title = "Region") +
  #scale_fill_viridis_d(aesthetics = "fill", option = "H", direction = -1) +
  scale_fill_manual(values = value_colors, na.translate = F) + 
   labs(fill = "Region") +
  hexpand(.2) +
  vexpand(vertical_expansion) +
